import React, {FC, useEffect, useState} from "react";
import NoticeModal from "./noticeModal";
import useSWRMutation from "swr/mutation";
import {NoticeType} from "../../interfaces/notice";
import {apiEndpoints} from "../../api/apiEndpoints";
import {editNotice} from "../../api/apiActions";
import {useRouter} from "next/router";
import {useSWRConfig} from "swr";
import {InputField} from "../directory/inputField";
import {isNoticeTitleNotUnique} from "../../utils/notice";
import {enqueueSnackbar} from "notistack";

export type NoticePropsType = {
    notice: NoticeType,
    noticesList: NoticeType[]
}
export const Notice: FC<NoticePropsType> = ({notice, noticesList}) => {
    const {title, id} = notice;

    const {query: {id: queryId = [], noticeId}, push} = useRouter();
    const {mutate} = useSWRConfig()
    const {trigger} = useSWRMutation<NoticeType>(apiEndpoints.notice(id), editNotice);

    const [showNoticeModal, setShowNoticeModal] = useState(false);
    const [isEditing, setIsEditing] = useState(false);
    const [noticeTitle, setNoticeTitle] = useState(title);
    const [isSelected, setIsSelected] = useState(false);
    const [isTitleDuplicated, setHasTitleDuplicated] = useState(false);

    const isTitleNotUnique = isNoticeTitleNotUnique(noticesList, id, noticeTitle);
    const errorMessage = "The note with this name is already taken.";

    useEffect(() => {
        if (noticeId) {
            if (Number(noticeId) === Number(id)) {
                setIsSelected(true);
                return;
            }
        }
        setIsSelected(false);
    }, [noticeId, id]);

    useEffect(() => {
        if (isTitleDuplicated) {
            enqueueSnackbar(errorMessage, {variant: 'error', autoHideDuration: 2000})
        }
    }, [isTitleDuplicated]);

    const onSaveClickHandler = async (note) => {
        setNoticeTitle(note.title);
        await trigger({...note, directoryId: Number(queryId.slice(-1))});
        await mutate(apiEndpoints.notices);
    }

    const onEditFieldKeyDownHandler = async (event) => {
        if (event.key === "Enter" || event.key === "Escape") {
            event.target.blur();
            setIsEditing(false);

            if (isTitleNotUnique) {
                setHasTitleDuplicated(true);
                setIsEditing(false);
                setNoticeTitle(title);
                return;
            }

            if (event.target.value.trim() !== title.trim() && event.target.value.trim() !== "") {
                await trigger({...notice, title: event.target.value.trim()});
                await mutate(apiEndpoints.notices);
            }
        }
    }

    const onEditFieldBlurHandler = async (event) => {
        if (isTitleNotUnique) {
            setHasTitleDuplicated(true);
            setIsEditing(false);
            setNoticeTitle(title);
            return;
        }

        setHasTitleDuplicated(false);
        setIsEditing(false);

        if (event.target.value.trim() !== title.trim() && event.target.value.trim() !== "") {
            await trigger({...notice, title: event.target.value.trim()});
            await mutate(apiEndpoints.notices);
        }
    }

    const onClickHandler = () => {
        const pathname = queryId.join('/');
        push({pathname, query: {noticeId: id}}, undefined, {shallow: true});
    }

    return (
        <>
            <div className={`flex flex-col items-center rounded-md p-2 ${isSelected && 'bg-blue-100'}`} onClick={onClickHandler}>
                <div onDoubleClick={() => setShowNoticeModal(true)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5"
                         stroke="currentColor" className="w-20 h-20">
                        <path strokeLinecap="round" strokeLinejoin="round"
                              d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/>
                    </svg>
                </div>
                <div>
                    {
                        isEditing ?
                            <InputField
                                name="noticeTitle"
                                value={noticeTitle}
                                onChange={setNoticeTitle}
                                onBlur={onEditFieldBlurHandler}
                                onKeyDown={onEditFieldKeyDownHandler}
                                className='w-auto'
                            />
                            : <h5
                                className="truncate text-ellipsis max-w-[100px]"
                                onClick={() => setIsEditing(true)}>
                                {title}
                            </h5>
                    }
                </div>
            </div>
            {
                showNoticeModal && (
                    <NoticeModal notice={notice} setShowModal={setShowNoticeModal} onSave={onSaveClickHandler}/>
                )
            }
        </>
    )
}